...
from util import stkwt_trnd

logging.config.fileConfig(fname='../config/api_log.conf', disable_existing_loggers=False)
...
class LongShort:
    def __init__(self):
        ...
        # Live Trade
        #self.alpaca = tradeapi.REST(API_KEY, API_SECRET, BASE_URL, 'v2')
        #Get the top trends from stockwits
        stockUniverse = stkwt_trnd.get_trending_stocks()
        logger.info("top tickers from stockwits ",stockUniverse)
    ...
    def run(self):
        ...
        while True:
            ...
            if (self.timeToClose < (60 * MINS_TO_CLOSE)):
                # Close all positions when MINS_TO_CLOSE minutes til market close.
                logger.info("Market closing soon.  Closing positions in {} mins".format(MINS_TO_CLOSE))
                ...
                logger.info("Sleeping until market close in {} minutes).".format(MINS_TO_CLOSE))
                time.sleep(60 * MINS_TO_CLOSE)
            else:
    ...
    def awaitMarketOpen(self):
        ...
        while (not isOpen):
            ...
            logger.info(str(timeToOpen) + " minutes til market open.")
            time.sleep(60)
            isOpen = self.alpaca.get_clock().is_open
    ...
    def rebalance(self):
        ...
        logger.info("We are taking a long position in: " + str(self.long))
        logger.info("We are taking a short position in: " + str(self.short))
        ...
        for position in positions:
            ...
            respSendBOLong = []
            tSendBOLong = threading.Thread(target=self.sendBatchOrder,
                                           args=[self.qLong, self.long, "buy", respSendBOLong])
            tSendBOLong.start()
            tSendBOLong.join()
    ...
    def submitOrder(self, qty, stock, side, resp):
        if qty > 0:
            try:
                ...
                logger.info("Market order of | " + str(qty) + " " + stock + " " + side + " | completed.")
                resp.append(True)
            except:
                logger.info("Order of | " + str(qty) + " " + stock + " " + side + " | did not go through.")
                resp.append(False)
        else:
            logger.info("Quantity is 0, order of | " + str(qty) + " " + stock + " " + side + " | not completed.")
            resp.append(True)
